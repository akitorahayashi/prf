...
impl InspectionInputs {
    ...
    pub fn protected_paths(&self) -> Result<Vec<PathBuf>, AppError> {
        let mut lexical_paths = vec![self.scope.root().to_path_buf()];
        ...
        for path in lexical_paths {
            ...
            match path.canonicalize() {
                ...
                Err(source) => {
                    return Err(AppError::path_operation("resolve protected path", path, source));
                }
            }
        }
        Ok(paths)
    ...
    pub fn for_test(scope: Scope) -> Self {
        let working_directory = scope.root().to_path_buf();
...
impl Discovery {
    pub fn inspect(
        ...
        match self {
            Self::Rules(rules) => Ok(inspect_rules(target, inputs.scope(), rules)),
...
impl Inspection {
    pub fn diagnostic(message: impl Into<String>) -> Self {
        Self { candidates: Vec::new(), listings: Vec::new(), diagnostics: vec![message.into()] }
    }
}

pub(crate) fn inspect_external_path(
    target: TargetId,
    inputs: &InspectionInputs,
    path: PathBuf,
) -> Result<Inspection, AppError> {
...
fn inspect_path(target: TargetId, path: PathBuf) -> Inspection {
    ...
    match path.symlink_metadata() {
        ...
        Err(error) => inspection
            .diagnostics
            .push(format!("Unable to inspect cleanup entry {}: {error}", path.display())),
    }
    inspection
}

fn inspect_rules(target: TargetId, scope: &Scope, rules: &'static [Rule]) -> Inspection {
    let mut inspection = Inspection::default();
    ...
    if let Some(home) = scope.home() {
        inspect_home_paths(target, home, rules, &mut inspection, &mut candidate_paths);
    }
...
fn inspect_root(
    ...
    let root = scope.root();
    if !root.exists() {
        inspection.diagnostics.push(format!("Scan root does not exist: {}", root.display()));
        return;
    }
    ...
    while let Some(result) = walker.next() {
        let entry = match result {
            ...
            Err(error) => {
                inspection
                    .diagnostics
                    .push(format!("Unable to inspect {:?}: {error}", error.path()));
                continue;
            }
        };
            ...
            match rule {
                Rule::DirectoryNames { names, parent_marker } => {
                    ...
                    if parent_marker.is_some_and(|marker| {
                        !entry.path().parent().is_some_and(|parent| parent.join(marker).is_file())
                    }) {
                        continue;
                    }
...
fn add_classified_path(target: TargetId, path: PathBuf, inspection: &mut Inspection) {
    let metadata = match path.symlink_metadata() {
        ...
        Err(error) => {
            inspection
                .diagnostics
                .push(format!("Unable to classify cleanup entry {}: {error}", path.display()));
            return;
        }
    };
...
mod tests {
    ...
    fn directory_rule_produces_candidates_and_listings_from_one_inspection() {
        ...
        let inspection = inspect_rules(TEST_TARGET, &scope, RULES);
    ...
    fn missing_root_is_an_explicit_diagnostic() {
        ...
        assert_eq!(
            ...
            vec![format!("Scan root does not exist: {}", missing.display())]
        );
    }
    ...
    fn home_rules_are_excluded_from_current_mode() {
        ...
        let default_inspection = inspect_rules(TEST_TARGET, &default_scope, RULES);
        ...
        let current_inspection = inspect_rules(TEST_TARGET, &current_scope, RULES);
